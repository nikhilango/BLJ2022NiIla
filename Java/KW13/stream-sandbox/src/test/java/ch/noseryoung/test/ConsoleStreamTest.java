...
import static ch.noseryoung.main.NintendoConsoles.ALL_CONSOLES;
...
public class ConsoleStreamTest {
    ...
    public void test_GetString_Played() {
        String actual = ALL_CONSOLES.stream().map(gameConsole -> "I have played on a " + gameConsole.getName() + "!").collect(Collectors.joining("\n"));
    ...
    public void test_GetString_PlayedOtherNames() {
        String actual = ALL_CONSOLES.stream().map(gameConsole -> {
            List<String> otherNames = gameConsole.getOtherNames();
            if (otherNames.isEmpty()){
                return "I have played on a " + gameConsole.getName() + "!";
            }
            else {
                return "I have played on a " + String.join("/", otherNames.get(0)) + "!";
            }
    ...
    public void test_GetPortables_SortedByUnitsSold() {
        List<GameConsole> actual = PORTABLES.stream().sorted(Comparator.comparing(GameConsole::getUnitsSoldWorldwide)).toList();
    ...
    public void test_GetHomeConsoles_ReleasedBeforeAndrew() {
        LocalDate andrewBD = LocalDate.parse("1997-10-20");
        List<GameConsole> actual = HOME_CONSOLES.stream().filter(gameConsole -> gameConsole.getInitialRelease().isBefore(andrewBD)).toList();
    ...
    public void test_GetAllConsoles_HaveMarioGameAsBestSeller() {
        List<GameConsole> actual = ALL_CONSOLES.stream().filter(gameConsole -> gameConsole.getBestSellingGame().contains("Mario")).toList();
    ...
    public void test_GetString_BestSellerOfBestSellingConsole() {
        String actual = String.valueOf(ALL_CONSOLES.stream().max(Comparator.comparingInt(GameConsole::getUnitsSoldWorldwide)).get().getBestSellingGame());
        int most = 0;

        /*
        for (int i = 0; i < ALL_CONSOLES.size(); i++) {
            if (ALL_CONSOLES.get(i).getUnitsSoldWorldwide() > ALL_CONSOLES.get(most).getUnitsSoldWorldwide()) {
                most = i;
            }
        }

        actual = ALL_CONSOLES.get(most).getBestSellingGame();


        for (int i = 0; i < ALL_CONSOLES.size(); i++) {
            if (ALL_CONSOLES.get(i).getUnitsSoldWorldwide() > most) {
                most = ALL_CONSOLES.get(i).getUnitsSoldWorldwide();
            }
        }
        for (int i = 0; i < ALL_CONSOLES.size(); i++){
            if (ALL_CONSOLES.get(i).getUnitsSoldWorldwide() == most){
                actual = ALL_CONSOLES.get(i).getBestSellingGame();
                break;
            }
        }
    */
    ...
    public void test_GetBoolean_AllConsolesDevelopedByNintendo() {
        boolean actual = ALL_CONSOLES.stream().allMatch(gameConsole -> gameConsole.getDeveloper().equals("Nintendo"));
        assertTrue(actual);
    }
    ...
    public void test_GetBoolean_ConsoleExistsLessThanMillionSold() {
        boolean actual = ALL_CONSOLES.stream().anyMatch(gameConsole -> gameConsole.getUnitsSoldWorldwide()<1000000);
        assertTrue(actual);
    }
    ...
    public void test_GetConsole_ExistsLessThanMillionSold() {
        GameConsole actual = ALL_CONSOLES.stream().filter(gameConsole -> gameConsole.getUnitsSoldWorldwide()<1000000).findFirst().get();
    ...
    public void test_GetBoolean_NoConsoleReleasedInJanuary() {
        boolean actual = ALL_CONSOLES.stream().noneMatch(gameConsole -> gameConsole.getInitialRelease().getMonth().equals(1));
        assertTrue(actual);
    }
    ...
    public void test_GetInt_ConsolesContainGameInName() {
        long actual = ALL_CONSOLES.stream().filter(ALL_CONSOLES -> ALL_CONSOLES.getName().contains("Game")).count();
        /*for (int i = 0; i < ALL_CONSOLES.size(); i++){
            if (ALL_CONSOLES.get(i).getName().contains("Game")){
                actual++;
            }
        }*/
    ...
    public void test_GetInt_ConsolesContainBoyInName() {
        long actual = ALL_CONSOLES.stream().filter(ALL_CONSOLES -> ALL_CONSOLES.getName().contains("Boy")).count();
    ...
    public void test_GetInt_ConsolesContainGameAndBoyInName() {
        long actual = ALL_CONSOLES.stream().filter(ALL_CONSOLES -> ALL_CONSOLES.getName().contains("Game") && ALL_CONSOLES.getName().contains("Boy")).count();
    ...
    public void test_GetPortables_NoOtherNamesAvailable() {
        List<GameConsole> actual = PORTABLES.stream().filter(gameConsole -> gameConsole.getOtherNames() == null).toList();
    ...
    public void test_GetHomeConsole_LatestGeneration() {
        GameConsole actual = ALL_CONSOLES.stream().max(Comparator.comparingInt(GameConsole::getGeneration)).get();
    ...
    public void test_GetString_ConsoleNameAndReleaseInNovemberSortedByDay() {
        String actual = ALL_CONSOLES.stream().filter(gameConsole -> gameConsole.getInitialRelease().getMonthValue() == 11).sorted(Comparator.comparingInt(gameConsole -> gameConsole.getInitialRelease().getDayOfMonth())).map(gameConsole -> gameConsole.getInitialRelease() + " " + gameConsole.getName()).collect(Collectors.joining("\n"));
        ...
        /**
         * Test:        Get String of pattern "<Console name> <units sold world wide>" sorted by popularity and only the top 3
         * Expected:    String as seen below.
         */
    @Test
    public void test_GetString_ConsoleNameAndUnitsSoldWorldWideSortedByPopularity(){
        String actual = "";
        String expected = """
                Nintendo DS 154020000
                Wii U 13560000
                Switch 107650000""";
        assertEquals(expected, actual);
    }
    @Test
    public void test_GetString_LeastSoldConsoleFromPortableConsolesBestseller(){
        String actual = ALL_CONSOLES.stream().min(Comparator.comparingInt(GameConsole::getUnitsSoldWorldwide)).get().getBestSellingGame();
        String expected = "Wario Land";
        assertEquals(expected, actual);
    }
}
